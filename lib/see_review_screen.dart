...
// Strona wyświetlająca wszystkie recenzje
class SeeReviewPage extends StatefulWidget {
  const SeeReviewPage({super.key});
...
class _SeeReviewPageState extends State<SeeReviewPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Recenzje Słuchawek'),
      ),
      body: Column(
        children: [
          Padding(
            ...
            child: StreamBuilder<DatabaseEvent>(
              ...
              builder: (context, snapshot) {
                ...
                allReviews = data.entries.map((entry) {
                  ...
                  String displayUser = userName.isNotEmpty ? userName : userEmail;

                  List<Map<String, dynamic>> freqList = [];
                  if (val['frequencyResponse'] != null && val['frequencyResponse'] is List) {
                    freqList = (val['frequencyResponse'] as List)
                        .map((item) => {
                      'frequency': double.tryParse(item['frequency'].toString()) ?? 0.0,
                ...
                return filteredReviews.isEmpty
                    ? const Center(child: Text('Brak recenzji do wyświetlenia'))
                    : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        title: Text('${review['brand']} ${review['model']}'),
                        subtitle: Text(shortReview),
                        trailing: Text('User: ${review['displayUser']}',
                            style: const TextStyle(fontSize: 12)),
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => FullReviewPage(
                                brand: review['brand'],
                                model: review['model'],
                                review: review['review'],
                                images: review['images'],
                                user: review['displayUser'],
                                frequencyResponse: review['frequencyResponse'] ?? [],
                              ),
                            ),
                          );
                        },
  ...
  void _applyFilters() {
    filteredReviews = allReviews.where((review) {
      ...
      final matchesType = selectedType == null || review['type'] == selectedType;
      final matchesTransducer = selectedTransducer == null || review['transducer'] == selectedTransducer;
      final matchesImpedance = selectedImpedance == null || review['impedance'] == selectedImpedance;
      final matchesSound = selectedSound == null || review['sound'] == selectedSound;
      final matchesBrand = selectedBrand == null || review['brand'] == selectedBrand;
...
class FullReviewPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final reviewId = "$brand-$model"; // Unikalny ID dla komentarzy
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text("Frequency Response:", style: TextStyle(fontWeight: FontWeight.bold)),
            FrequencyResponseDisplay(freqData: frequencyResponse),
            const SizedBox(height: 20),
            CommentsWidget(reviewId: reviewId), // sekcja komentarzy
          ],
        ),
      ),
    );
...
// Wiget do wyświetlania frequency response
